	...
	describe('Services', function() {
		...
		describe('The xml schema parser', function() {
			...
			it('should set a default target namespace when one is available', function() {
				var schemaWithoutTargetNamespace = xsd.parse(getXsd('simpleElement'));
				expect(schemaWithoutTargetNamespace.namespace).toBe(null);

				var schemaWithTargetNamespace = xsd.parse(getXsd('simpleElement_targetNamespace'));
				expect(schemaWithTargetNamespace.namespace).toBe('http://formz.com/ANamespace');
			});
			...
			describe('when parsing validation', function() {
				it("should parse attributes' optionality", function() {
					...
					attribute = rootElement.children[1];
					expect(attribute.required).toBeFalsy();

					attribute = rootElement.children[2];
					expect(attribute.required).toBeFalsy();
				});
			});
